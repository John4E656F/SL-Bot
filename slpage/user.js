...
import basicCards from '../data/basicCards.js';
import moment from 'moment';

const isPlayable = (username, card) => {
  if (
    (card.delegated_to === username || card.delegated_to === null || card.delegated_to === '') && //card delegated to player or owned
    (card.market_listing_status === null ||
      card.market_listing_status === '' ||
      (card.market_listing_status != null && card.delegated_to === username)) && // not listed to market
    (card.unlock_date === null || card.unlock_date === '') && // not locked
    (card.last_used_player === username ||
      card.last_used_player === null ||
      (card.last_used_player !== username && moment.duration(moment().diff(card.last_used_date)).days() > 1)) && // rented not locked
...
const isRented = (username, card) => {
  if (
    card.delegated_to === username &&
    card.player !== username && //card is delegated to player
    (card.unlock_date === null || card.unlock_date === '') &&
    (card.last_used_player === username ||
      card.last_used_player === null ||
      (card.last_used_player !== username && moment.duration(moment().diff(card.last_used_date)).days() > 1))
  ) {
    // not locked
    return true;
  } else {
...
export const getPlayerCards = (username) =>
  fetch(`https://api2.splinterlands.com/cards/collection/${username}`, {
    credentials: 'omit',
...
export const getRentedCards = (username) =>
  fetch(`https://api2.splinterlands.com/cards/collection/${username}`, {
    credentials: 'omit',
    headers: { accept: 'application/json, text/javascript, */*; q=0.01' },
    referrer: `https://splinterlands.com/?p=collection&a=${username}`,
